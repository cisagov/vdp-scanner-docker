...
```console
docker run cisagov/vdp-scanner:0.2.0
```
...
1. Create a `docker-compose.yml` file similar to the one below to use [Docker Compose](https://docs.docker.com/compose/).
    ...
    services:
      vdp-scanner:
        image: 'cisagov/vdp-scanner:0.2.0'
        volumes:
          - .:/task/host_mount
    ```
...
1. Pull the new image:
    ...
    ```console
    docker pull cisagov/vdp-scanner:0.2.0
    ```
...
a version tag (e.g. `:0.2.0`).

| Image:tag | Description |
|-----------|-------------|
|`cisagov/vdp-scanner:0.2.0`| An exact release version. |
